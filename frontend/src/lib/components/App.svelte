      ...
      >
        {#if stateCtx.profile}
          {#if stateCtx.profile?.name || stateCtx.profile?.lastname}
            <div>{stateCtx.profile.name ?? ''} {stateCtx.profile.lastname ?? ''}</div>
          {/if}
          ...
          <div class="flex gap-2">
            ...
            {#if stateCtx.profile.cv}
              <a href={stateCtx.profile.cv} target="_blank" rel="noopener noreferrer" aria-label={stateCtx.profile.cv}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"
                  ><path
